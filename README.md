...
- `电池`：放电使用 SMC `PPBR` 汇总电池轨并显示为负数；充电使用电池包总电压与充电器电流并显示为正数。
...
电脑功率来自 SMC `PSTR`，电源输入来自 SMC `PDTR`，电池放电功率来自 SMC `PPBR`；这些传感器和菜单通常约每秒更新。充电功率使用电池包总电压与充电器电流计算。这些数值适合观察功率方向和量级，不应当作精密功率计。

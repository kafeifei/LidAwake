...
- 通过 Sparkle 提供应用内自动更新：默认每天在后台检查一次，发现新版本后自行下载、安装并重新启动应用，全程不弹窗、不需要点击（安装只在菜单未打开时进行）。只有在关闭自动安装或系统不允许自动安装时，才会退回到提示方式：菜单栏电池图标右上角显示一个红点，菜单项变成“有新版本 X.Y.Z…”，点击它打开更新对话框，看过之后红点消失。自动安装可以在菜单里的“自动安装更新”中开关；手动“检查更新…”仍然直接弹出对话框。
